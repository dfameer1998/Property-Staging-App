...
public class MainActivity extends androidx.activity.ComponentActivity {
 ...
 @Override public void onCreate(Bundle state){super.onCreate(state);getOnBackPressedDispatcher().addCallback(this,new androidx.activity.OnBackPressedCallback(true){@Override public void handleOnBackPressed(){if(busy)return;if(space!=null)loadSpaces();else if(project!=null)loadProjects();else finish();}});getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);if(api.signedIn())loadProjects();else login();}
 ...
 @Override protected void onActivityResult(int request,int result,Intent data){super.onActivityResult(request,result,data);if(result!=RESULT_OK||data==null||space==null)return;String id=space.optString("id");
  ...
  if(request==PICK&&data.getData()!=null){Uri uri=data.getData();work("Uploading…",()->{String mime=getContentResolver().getType(uri);if(mime==null||!java.util.Arrays.asList("image/jpeg","image/png","video/mp4","video/quicktime").contains(mime))throw new IOException("Choose a JPEG, PNG, MP4, or MOV file.");try(InputStream in=getContentResolver().openInputStream(uri);ByteArrayOutputStream out=new ByteArrayOutputStream()){if(in==null)throw new IOException("Could not open this file.");byte[] buf=new byte[8192];int n;while((n=in.read(buf))!=-1){if(out.size()+n>10*1024*1024)throw new IOException("Choose a file under 10 MB.");out.write(buf,0,n);}api.upload(out.toByteArray(),mime,id);}},this::loadSpace);}
 }
